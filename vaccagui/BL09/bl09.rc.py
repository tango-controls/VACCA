...
TITLE = 'VACCA: The VACuum Control Application for BL09-Mistral'
LOG_LEVEL = 'WARNING'
...
def get_mistral_grid():
    ...
    tg.setModel('(BL09|FE09)/VC/(VG|IP)CT-*/(P[12])')
    #tg.showAttributeLabels(False)
    #tg.showAttributeUnits(False)
    return tg

def POST_HOOK(ui):
    print 'In bl09.rc.py POST_HOOK method ...'
    try:
...
SYNOPTICS = SortedDict([
       ('ALL',(wdir('BL09/BL09.jdw'),get_mistral_grid)),
...
def get_default_tree():
    ...
    for dev in tau.Database().get_device_member('FE09/VC/*'): 
        tree['BL09/VC/ALL']['FE09/VC/%s'%dev]={}
    tree['BL09/VC/ALL']['BL09/CT/EPS-PLC-01']={}
    tree['BL09/VC/ALL']['BL09/CT/EPS-PLC-01-MBUS']={}
    tree['BL09/VC/ALL']['BL09/CT/ALARMS']={}
    return tree
...
MENUS = [
    ('Tools',[
        ...
        ('Archiving Webserver',lambda:os.system('konqueror ctbl09arch01 &'),None), \
        ]),
    ]
