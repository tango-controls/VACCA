...
COMPOSER = DEVICE = 'SR/EPS/ALARMS'
from taurus.qt.qtgui.tree import TaurusDevTree
TaurusDevTree.TRACE_ALL = True

EXTRA_DEVICES = fandango.get_matching_devices('*/(eps*plc-01|pnv|brtu|fsotr)*')
...
# The grid does not show up properly, shows also modbus devices and does not allow to select into panels    
NOGRID = {
    'column_labels': 
        ','.join((
        'Q1:(SR01|SR02|SR03|SR04|BO01|FE01|FE02|FI03|FE04)*',
        'Q2:(SR05|SR06|SR07|SR08|BO02|LI|LT|FE09|FE11|FE13)*',
        'Q3:(SR09|SR10|SR11|SR12|BO03|FE22|FE24)*',
        'Q4:(SR13|SR14|SR15|SR16|BO04|BT|FE29|FE34)*',
        )),
