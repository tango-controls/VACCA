...
def p_error(p):
    #p.lexer.log.error
    print("[%d]: Syntax error in input [%s]" % (p.lexer.lineno,(str(p))))
...
def p_jdfile(p):
    ...
    if p[0] is None:
        #p.parser.log.info
        print("[%d]: Unable to create Scene" % p.lexer.lineno)
...
def p_single_element(p):
    ...
    factory = p.parser.factory
    #p.parser.log.debug
    #print('ret = factory.getObj(%s,%s)'% (str(p[1]),str(p[3])))
    ret = factory.getObj(p[1],p[3])
 
    if ret is None:
        #p.parser.log.info
        print("[%d]: Unable to create obj '%s'" % (p.lexer.lineno,p[1]))
