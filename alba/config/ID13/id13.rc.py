...
TITLE = 'VACCA: The VACuum Control Application for ID13'
LOG_LEVEL = 'WARNING'
FAST_MODE = True
...
def get_default_tree():
    ...
    composer = 'ID13/CT/ALARMS'
    tree[composer] = dict((d.upper(),{}) for d in get_matching_devices('ID13/*/%s*'%(families)) if d.split('/')[-1]!='ALL')
...
TOOLBAR = [
        ...
        ('','',None),
        ('EPS',wdir('image/equips/icon-eps.gif'), lambda:os.system('epsGUI_ID13 &')),
        ('','',None),
        ]
...
MENUS = [
    ('Tools',[
        ('EPS Gui',lambda:os.system('epsGUI_ID13 &'),None), \
