...
print '>'*20+' Loading config for plcs'
...
EXTRA_DEVICES = [d for d in fandango.get_matching_devices('*plc*') if 'mbus' not in d and 'dserver' not in d]+['sr/main/machinestatus']
...
GRID = {
        'row_labels': ','.join(sorted(set(d.split('/')[0].upper() for d in EXTRA_DEVICES if not d.startswith('sr/')))),
        'delayed': False,
        'frames': False,
        'model': '*/*/*plc*/(state|plc_config_status|plc_config_alrm|plc_config_warn)$',#'fe*/di/xbpmlocum*/(state|x|y|I)*',
        'column_labels': 'State,Status,Alrm,Warn',
    }
